...
<div id="tribe-events-content" class="upcoming">
	...
	<?php if(!tribe_is_day()): // day view doesn't have a grid ?>
		<div id='tribe-events-calendar-header' class="line">
	...
	<?php while ( have_posts() ) : the_post(); ?>
		...
		<div id="post-<?php the_ID(); ?>" <?php post_class('line'); ?> itemscope itemtype="http://schema.org/Event">
			<?php if ( tribe_is_new_event_day() && !tribe_is_day() && !tribe_is_multiday() ) : ?>
				<h4 class="m0 em2 normal"><?php echo tribe_get_start_date( null, false ); ?></h4>
			<?php endif; ?>
			<?php if( !tribe_is_day() && tribe_is_multiday() ) : ?>
				<h4 class="m0 em2 normal"><?php echo tribe_get_start_date( null, false ); ?> – <?php echo tribe_get_end_date( null, false ); ?></h4>
			<?php endif; ?>
			<?php if ( tribe_is_day() && $first ) : $first = false; ?>
				<h4 class="m0 em2 normal"><?php echo tribe_event_format_date(strtotime(get_query_var('eventDate')), false); ?></h4>
			<?php endif; ?>
			<?php the_title('<h2 class="em2 m0" itemprop="name"><a href="' . tribe_get_event_link() . '" title="' . the_title_attribute('echo=0') . '" rel="bookmark">', '</a></h2>'); ?>
